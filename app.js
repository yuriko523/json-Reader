...
const Storage = {
  ...
  async save(data) {
    ...
    // 2. Cache API（大数据通道，几百 MB 容量，MIUI 一般不碰）
    try {
      const cache = await caches.open(this.CACHE_NAME);
      await cache.put('chatData', new Response(raw, {
        headers: { 'Content-Type': 'application/json' }
      }));
      // 清理旧 localStorage 碎片
      localStorage.removeItem('jsonReaderData');
      const size = (raw.length / 1024 / 1024).toFixed(1);
      console.log(`[存储] ✅ Cache API: ${data.items?.length || 0} 条 (${size}MB)`);
      return;
    } catch (e) {
      console.error('[存储] Cache API 写入失败:', e);
    }

    // 3. IndexedDB（最后兜底）
    console.log('[存储] 回退 IndexedDB…');
  ...
  async load() {
    ...
    if (raw) {
      try {
        ...
        return data;
      } catch (e) { console.warn('[存储] 解析失败'); }
    }

    // 2. Cache API
    try {
      const cache = await caches.open(this.CACHE_NAME);
      const resp = await cache.match('chatData');
      if (resp) {
        const text = await resp.text();
        const data = JSON.parse(text);
        console.log(`[存储] Cache API 加载: ${data.items?.length || 0} 条`);
        return data;
      }
    } catch (e) { console.warn('[存储] Cache API 读取失败:', e); }
  ...
  async clear() {
    localStorage.removeItem('jsonReaderData');
    try { await caches.delete(this.CACHE_NAME); } catch (_) {}
...
function renderList() {
  ...
  dom.listWrap.style.display   = showAll ? '' : 'none';
  dom.folderWrap.style.display = showAll ? 'none' : '';
  document.getElementById('list-toolbar').style.display = showAll ? 'flex' : 'none';
  ...
  dom.listWrap.addEventListener('touchstart', (e) => {
    ...
    lpTimer = setTimeout(async () => {
      ...
      const action = await showCardMenu(idx);
      if (action === 'delete') {
        const confirmed = await confirmDeleteItem(idx);
        if (confirmed) {
          state.items.splice(idx, 1);
          removeItemFromFolders(idx);
          await saveState();
          renderList();
          showToast(`已删除第 ${idx + 1} 条`);
        }
      } else if (action === 'folder') {
        await showFolderPicker(idx);
        renderList();
      }
    }, 600);
  }, { passive: true });
...
function removeItemFromFolders(idx) {
  state.folders.forEach(f => {
    f.itemIndices = f.itemIndices.filter(i => i !== idx);
  });
  state.folders = state.folders.filter(f => f.itemIndices.length > 0);
}

// 卡片长按菜单
function showCardMenu(index) {
  return showModal(`
    <div class="modal-icon">📋</div>
    <h3>操作</h3>
    <p>#${index + 1} ${truncate(state.items[index]?.bookmark || state.items[index]?.userContent || '', 30)}</p>
    <div class="modal-actions">
      <button class="btn-danger"  data-action="delete">🗑 删除此条</button>
      <button class="btn-primary" data-action="folder">📁 加入文件夹</button>
      <button class="btn-ghost"   data-action="cancel">取消</button>
    </div>
  `);
}
...
// 文件夹选择弹窗
async function showFolderPicker(itemIdx) {
  const existingIds = state.folders.filter(f => f.itemIndices.includes(itemIdx)).map(f => f.id);
  let selected = new Set(existingIds);

  // 循环直到用户确定或取消
  let done = false;
  while (!done) {
    const folderList = state.folders.map(f => `
      <label style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
        <input type="checkbox" class="folder-cb" value="${f.id}" ${selected.has(f.id) ? 'checked' : ''}>
        <span style="font-size:0.9375rem">📁 ${esc(f.name)} <span style="color:var(--text-secondary);font-size:0.8125rem">(${f.itemIndices.length}条)</span></span>
      </label>
    `).join('');
    ...
    const html = `
      ...
      <div style="text-align:left;max-height:250px;overflow-y:auto;margin-bottom:12px">
        ${folderList || '<p style="color:var(--text-secondary);font-size:0.875rem;padding:12px 0">还没有文件夹，新建一个吧</p>'}
      </div>
      <div class="modal-actions">
        <button class="btn-primary" data-action="new">＋ 新建文件夹</button>
        <button class="btn-primary" data-action="confirm">确定</button>
        <button class="btn-ghost"  data-action="cancel">取消</button>
      </div>
    `;

    const action = await showModal(html);
    ...
    if (action === 'new') {
      const name = await showNewFolderPrompt();
      if (name) {
        const newId = 'f_' + Date.now();
        state.folders.push({ id: newId, name, itemIndices: [] });
        selected.add(newId);
        // 重新渲染弹窗（循环继续）
      }
      ...
      state.folders.forEach(f => {
        if (selected.has(f.id)) {
          if (!f.itemIndices.includes(itemIdx)) f.itemIndices.push(itemIdx);
        } else {
          f.itemIndices = f.itemIndices.filter(i => i !== itemIdx);
        }
      });
      await saveState();
      showToast('文件夹已更新');
...
function showNewFolderPrompt() {
  return new Promise((resolve) => {
    const html = `
      ...
      <h3>新建文件夹</h3>
      <p>输入文件夹名称</p>
      <input id="new-folder-input" class="setting-input" style="width:100%;box-sizing:border-box;margin-bottom:12px" placeholder="例如：工作、学习…" maxlength="30">
      <div class="modal-actions">
        <button class="btn-primary" data-action="confirm">创建</button>
        <button class="btn-ghost"  data-action="cancel">取消</button>
      </div>
    `;
    dom.modalBody.innerHTML = html;
...
function renderFolders() {
  const wrap = document.getElementById('folder-wrap');
  if (!wrap) return;

  if (state.folders.length === 0) {
    wrap.innerHTML = `<div class="empty-state" style="min-height:30vh"><div class="empty-icon">📁</div><p class="empty-title">还没有文件夹</p><p class="empty-desc">长按记录卡片可以创建文件夹</p></div>`;
    return;
  }

  wrap.innerHTML = state.folders.map(f => {
    const items = f.itemIndices
      .map(idx => state.items[idx])
      .filter(Boolean)
      .map((item, i) => {
        const preview = item.bookmark || truncate(item.userContent || '(空)', 40);
        const phStar = item.hasPHMarker ? '⭐ ' : '';
        const realIdx = f.itemIndices.filter(idx => state.items[idx])[i];
        return `<div class="folder-item" data-idx="${realIdx}">
          <div class="folder-item-preview">${phStar}${esc(preview)}</div>
          <div class="folder-item-meta">#${(state.sortOrder === 'asc' ? realIdx + 1 : state.items.length - realIdx)}</div>
        </div>`;
      }).join('');

    return `<div class="folder-card">
      <div class="folder-header" data-folder="${f.id}">
        <span class="folder-name">📁 ${esc(f.name)}</span>
        <span class="folder-meta">${items.length} 条 ▸</span>
      </div>
      <div class="folder-body" id="fb_${f.id}">
        <div class="folder-body-inner">${items || '<span style="color:var(--text-secondary);font-size:0.875rem">（无记录）</span>'}</div>
      </div>
    </div>`;
  }).join('');

  // 手风琴展开/折叠
  wrap.querySelectorAll('.folder-header').forEach(header => {
    header.addEventListener('click', () => {
      const fid = header.dataset.folder;
      const body = document.getElementById('fb_' + fid);
