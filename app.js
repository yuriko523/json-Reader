...
const Storage = {
  KEY: 'main',
  CHUNK_PREFIX: 'jr_chunk_',
  CHUNK_SIZE: 4 * 1024 * 1024,  // 4MB（localStorage 5MB 上限留余量）
  ...
  async save(data) {
    ...
    try {
      ...
      console.log(`[存储] ✅ 直接存储: ${data.items?.length || 0} 条`);
      return;
    } catch (e) {
    ...
    try {
      this._clearChunks();
      const totalChunks = Math.ceil(raw.length / this.CHUNK_SIZE);
      for (let i = 0; i < totalChunks; i++) {
        const chunk = raw.substring(i * this.CHUNK_SIZE, (i + 1) * this.CHUNK_SIZE);
        localStorage.setItem(this.CHUNK_PREFIX + i, chunk);
      }
      localStorage.setItem(this.CHUNK_PREFIX + 'count', String(totalChunks));
      localStorage.removeItem('jsonReaderData');
      localStorage.removeItem('jsonReader_storageType');
      console.log(`[存储] ✅ 分块存储: ${data.items?.length || 0} 条 = ${totalChunks} 块`);
      return;
    } catch (e) {
      console.error('[存储] 分块写入失败:', e);
    }
  ...
  async load() {
    ...
    if (raw) {
      try {
        ...
        console.log(`[存储] 加载: ${data.items?.length || 0} 条`);
        return data;
      } catch (e) { console.warn('[存储] 解析失败'); }
    }

    // 2. 分块 localStorage
    const chunkCount = parseInt(localStorage.getItem(this.CHUNK_PREFIX + 'count')) || 0;
    if (chunkCount > 0) {
      try {
        let full = '';
        for (let i = 0; i < chunkCount; i++) {
          const chunk = localStorage.getItem(this.CHUNK_PREFIX + i);
          if (!chunk) throw new Error(`块 ${i} 缺失`);
          full += chunk;
        }
        const data = JSON.parse(full);
        console.log(`[存储] 分块加载: ${data.items?.length || 0} 条 (${chunkCount} 块)`);
        return data;
      } catch (e) { console.warn('[存储] 分块解析失败:', e); }
    }

    // 3. IndexedDB（兜底）
    const type = localStorage.getItem('jsonReader_storageType');
    if (type === 'idb' || type === null) {
  ...
  _clearChunks() {
    const count = parseInt(localStorage.getItem(this.CHUNK_PREFIX + 'count')) || 0;
    for (let i = 0; i < count; i++) localStorage.removeItem(this.CHUNK_PREFIX + i);
    localStorage.removeItem(this.CHUNK_PREFIX + 'count');
  },
